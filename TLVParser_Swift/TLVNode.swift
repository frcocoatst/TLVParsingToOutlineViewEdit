...
var tagNamesDict = ["e1": "Container E1",
                    "9f1e": "Interface Device (IFD) Serial Number",
                    "ef": "Text2",
                    "df0d": "Fleet Issuer Action Code - Default",
                    "df7f": "no idea"]

// tagNamesDict[str ?? "unknown")
...
@objcMembers class TLVNode: NSObject {
    ...
    var displayName: String {
        // Pad the hex value with 0 so it outputs `0d` instead of just `d`
        // var str = tag.map { ("0" + String($0, radix: 16)).suffix(2) }.joined()
        let strName: String? = tagNamesDict[displayTag] ?? "unspecified"
